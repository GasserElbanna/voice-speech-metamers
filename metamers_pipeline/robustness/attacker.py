...
class AttackerModel(ch.nn.Module):
    ...
    def __init__(self, model, dataset):
        ...
        # If true, then runs the linear layer component
        self.run_linear_layer=False
...
#         if with_latent and self.run_linear_layer:
#             print('YO')
#             # last element of output is the dictionary of layer outputs
#             output[2]['linear_map_%s'%self.output_layer] = self.linear_outputs[self.output_layer]            

        return (output, inp)

#     def forward_hook_with_linear(self, layer_name):
#         def hook(module, input, output):
 #            self.selected_out[layer_name] = output
 #            out_mean_flattened = ch.flatten(ch.mean(output, dim=-1), start_dim=1)
 #            self.linear_outputs[layer_name] = self.component_linear(out_mean_flattened)
 #        return hook

    def set_up_forward_linear_hook(self, output_layer, 
                                   layer_linear_weights, 
                                   layer_linear_intercepts):
        self.run_linear_layer=True
        self.output_layer = output_layer
        self.selected_out = OrderedDict()
        self.linear_outputs = OrderedDict()
        self.fhooks = []

#         self.component_linear = ch.nn.Linear(in_features=layer_linear_weights.shape[-1],
#                                             out_features=1,
#                                             bias=True)
#         with ch.no_grad():
#             self.component_linear.weight.copy_(ch.tensor(layer_linear_weights))
#             self.component_linear.bias.copy_(ch.tensor(layer_linear_intercepts))

        self.model = LinearReadoutModelWrapper(self.model, output_layer,
                                                       layer_linear_weights, layer_linear_intercepts)
        self.attacker.model = self.model
#         for i_1,l_1 in enumerate(list(self._modules.keys())):
#             if l_1 == self.output_layer.split('.')[0]:
#                 output_layer_short =  self.output_layer.split('.')[1]
#                 self.model = LinearReadoutModelWrapper(getattr(self, l_1), output_layer_short,
#                                                                layer_linear_weights, layer_linear_intercepts)
#             for i_2,l_2 in enumerate(list(getattr(self, l_1)._modules.keys())):
#                 print('%s.%s'%(l_1,l_2))
#                 print(self.output_layer)
#                 if '%s.%s'%(l_1,l_2) == self.output_layer:
#                     print('YES')
#                     self.fhooks.append(getattr(getattr(self, l_1),l_2).register_forward_hook(self.forward_hook_with_linear('%s.%s'%(l_1,l_2))))


class LinearReadoutModelWrapper(ch.nn.Module):
    def __init__(self,
                 model,
                 output_layer,
                 layer_linear_weights,
        ...
        for i_1,l_1 in enumerate(list(self.model._modules.keys())):
            if l_1 == self.output_layer:
                print('YES')
                self.fhooks.append(getattr(self.model, l_1).register_forward_hook(self.forward_hook_with_linear(l_1)))
    ...
    def forward(self, x, **kwargs):
        ...
        if kwargs.get('with_latent', False):
            out[2]['linear_map_%s'%self.output_layer] = self.linear_outputs[self.output_layer]
        return out
