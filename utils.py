...
def load_audios(batch):
    ...
    batch["audio"] = torchaudio.load(wav_file)[0]
    batch["audio"] = batch["audio"].to('cuda')
    return batch

def extract_all_phones(sentences):
    all_text = [phone for sentence in sentences for phone in sentence]
    vocab = list(set(all_text))
    return vocab

def prepare_data(batch, feat_ext, tokenizer, sr):
    #load audio files
    wav_file = batch["wav_path"]
    batch["audio"] = torchaudio.load(wav_file)[0]
    # batch["audio"] = batch["audio"].to('cuda')
    ...
    inputs = feat_ext(audio, sampling_rate=sr)
    batch["input_values"] = inputs.input_values[0]
    batch["input_length"] = len(inputs.input_values[0])

    #tokenize phoneme labels
    # phones = [phone for phone in phones if phone != ' ']
    tokens = tokenizer.encode(phones)
    batch["labels"] = tokens
    batch["labels_length"] = len(tokens)

    return batch

def per(hypothesis, groundtruth):
    err = 0
    tot = 0
